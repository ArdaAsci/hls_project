...
extern "C" void matrixmul(char*, char*, short*, short*);
extern "C" void apatb_matrixmul_hw(volatile void * __xlx_apatb_param_a, volatile void * __xlx_apatb_param_b, volatile void * __xlx_apatb_param_res, volatile void * __xlx_apatb_param_res1) {
  ...
  // Collect __xlx_res1__tmp_vec
  vector<sc_bv<16> >__xlx_res1__tmp_vec;
  for (int j = 0, e = 9; j != e; ++j) {
    sc_bv<16> _xlx_tmp_sc;
    _xlx_tmp_sc.range(7, 0) = ((char*)__xlx_apatb_param_res1)[j*2+0];
    _xlx_tmp_sc.range(15, 8) = ((char*)__xlx_apatb_param_res1)[j*2+1];
    __xlx_res1__tmp_vec.push_back(_xlx_tmp_sc);
  }
  int __xlx_size_param_res1 = 9;
  int __xlx_offset_param_res1 = 0;
  int __xlx_offset_byte_param_res1 = 0*2;
  short* __xlx_res1__input_buffer= new short[__xlx_res1__tmp_vec.size()];
  for (int i = 0; i < __xlx_res1__tmp_vec.size(); ++i) {
    __xlx_res1__input_buffer[i] = __xlx_res1__tmp_vec[i].range(15, 0).to_uint64();
  }
  // DUT call
  matrixmul(__xlx_a__input_buffer, __xlx_b__input_buffer, __xlx_res__input_buffer, __xlx_res1__input_buffer);
...
// print __xlx_apatb_param_res1
  sc_bv<16>*__xlx_res1_output_buffer = new sc_bv<16>[__xlx_size_param_res1];
  for (int i = 0; i < __xlx_size_param_res1; ++i) {
    __xlx_res1_output_buffer[i] = __xlx_res1__input_buffer[i+__xlx_offset_param_res1];
  }
  for (int i = 0; i < __xlx_size_param_res1; ++i) {
    ((char*)__xlx_apatb_param_res1)[i*2+0] = __xlx_res1_output_buffer[i].range(7, 0).to_uint();
    ((char*)__xlx_apatb_param_res1)[i*2+1] = __xlx_res1_output_buffer[i].range(15, 8).to_uint();
  }
}
